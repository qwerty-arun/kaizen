...
<details>
  ...
  - CAD Modeling: Recreated the twisted connector
  - PCB Designing: GND copper region, top layer copper regions, rebuilding regions, changing the default settings
  - 3D printing course: Material Jetting technique
  - Read about the 3 Body problem: What if one of the bodies has very less mass? In that case, the system is stable, not chaotic.
  - Fermi Paradox: What if the barrier was in the past and we have already gone past it? What if it is in the future and we may not be able to cross it? May be most civilizations don't get to a point of interstellar travel because they self-destruct themselves. Are they trying to hide and don't want to interact with us purposefully? Who knows? 
  - SFML: Perfect game setup. Learnt many things: header files (precompiled headers), pointers usage, classes, this keyword (makes it easier to see what is part of the class and what's not), commenting at each and every function.
  - SFML (my game): Done with my asteroid game: added sprites to my asteroids, added sound effects for laser shooting etc. Perspective projection of a cube (normalizing using z coordinates).
  </details>
<details>
